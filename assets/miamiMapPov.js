Reveal.on("ready", async () => {
    ...
    const miami = await d3.json(
        "assets/shape_files/miami_2010_albersusa_topo.json",
        (json) => json
    );
    const projection = d3.geoMercator().translate([72250, 24100]).scale(51200);
    const path = d3.geoPath(projection);
    const rawData = await d3.csv("assets/data/miami_data.csv", (csv) => csv);
    const features = new Map(
        topojson
            .feature(miami, miami.objects.ipums_puma_2010)
    ...
    const blackData = rawData.map((obj) => ({
        id: obj.STATEFIP.padStart(2, "0") + obj.PUMA.padStart(5, "0"),
        position:
            features.get(
                obj.STATEFIP.padStart(2, "0") + obj.PUMA.padStart(5, "0")
        ...
        value: parseFloat(obj.p_pov) * 100,
    }));
    ...
    const length = d3.scaleQuantize(
        d3.extent(blackData.map((d) => d.value)),
        [4, 10, 18, 29, 43, 60, 79]
    );
    ...
    g.append("g")
        .selectAll("path")
        .data(topojson.feature(miami, miami.objects.ipums_puma_2010).features)
        .join("path")
        .attr("fill", (d) =>
...
EEs Due per Capita: ${format(
    ...
    g.append("g")
        ...
        .append("title")
        .text((d) => `Below Poverty Threshold: ${format(d.value)}%`);
